...
// Hub manages all WebSocket connections for presence.
type Hub struct {
...
// GetRoomClientCount returns the number of clients in a room (local only).
...
// GetConfig returns hub configuration.
	...
	for {
		...
		if err != nil {
			if websocket.IsUnexpectedCloseError(err, websocket.CloseGoingAway, websocket.CloseAbnormalClosure) {
				l := pkglog.L()
				l.Warn().Err(err).Msg("websocket unexpected close")
			}
			break
		}
